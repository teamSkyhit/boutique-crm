...
// Image Upload API — presigned URL flow (file goes directly to R2, not through server)
export const uploadAPI = {
  getSignedUrl: async (fileName, fileType, folder = 'products', token) => {
    const params = new URLSearchParams({ fileName, fileType, folder });
    return apiCall(`/api/upload/signed-url?${params}`, {
      method: 'GET',
      headers: { Authorization: `Bearer ${token}` },
    });
  },

  // PUT directly to R2 — must NOT include Authorization header
  uploadToR2: async (uploadUrl, file) => {
  ...
  uploadImage: async (file, token, folder = 'products') => {
    const signedRes = await uploadAPI.getSignedUrl(file.name, file.type, folder, token);
    if (!signedRes.success) {
      throw new Error(signedRes.error || signedRes.message || 'Failed to get upload URL');
    }
    const { uploadUrl, objectKey, publicUrl } = signedRes.data;
    await uploadAPI.uploadToR2(uploadUrl, file);
    return { data: { url: publicUrl, objectKey } };
  },
