...
export default function AddProductPage() {
  ...
  const { data: shelves = [], isLoading: loadingShelves } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!response.success) throw new Error(response.error || response.message || 'Failed to load shelf locations');
  ...
  const handleSingleProductSubmit = async () => {
    ...
    if (response.success) {
      ...
      toast.error(response.error || response.message || 'Failed to add product');
    }
  };
  ...
  const handleMultipleProductsSubmit = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        toast.error(response.error || response.message || 'Failed to create product with variants');
        logger.error('Product creation failed:', response.message);
      }
    } catch (error) {
